...
async function createChildJob(event: JobEvent): Promise<void> {
  ...
  logger.info("child_job_created", { parent: data.parent_job_id, child: childId });
}
...
async function onParsingCompleted(event: JobEvent): Promise<void> {
  ...
  logger.info("parsing_completed_received", { job_id: event.job_id, parsed: data.parsed, dropped: data.dropped_rubbish, failed: data.failed, part_count: data.part_s3_paths.length });
  
  const row = await getJob(event.job_id);
  if (!row) {
    logger.error("job_not_found", { job_id: event.job_id });
    return;
  }
  ...
  logger.info("transitioning_to_finalizing", { job_id: event.job_id, failed_ratio: failedRatio });
  ...
  logger.info("starting_finalization", { job_id: event.job_id, part_paths: data.part_s3_paths });
  
  try {
    const finalizeResult = await finalizeOutput(event.job_id, data.part_s3_paths, settings.DATA_BUCKET);
    logger.info("finalization_result", { job_id: event.job_id, failed: finalizeResult.failed, paths_count: finalizeResult.paths.length, error: finalizeResult.error });
    
    if (finalizeResult.failed) {
      logger.error("finalize_failed", { job_id: event.job_id, error: finalizeResult.error });
    ...
    logger.info("finalize_complete", { job_id: event.job_id, merged_paths_count: mergedPaths.length, merged_paths: mergedPaths });

    if (failedRatio > settings.FAILED_LINE_RATIO_THRESHOLD) {
      logger.warn("quality_gate_held", { job_id: event.job_id, failed_ratio: failedRatio, threshold: settings.FAILED_LINE_RATIO_THRESHOLD });
    ...
    if (mergedPaths.length === 0 && data.parsed > 0) {
      logger.warn("no_output_paths_with_parsed_data", { job_id: event.job_id, parsed: data.parsed, part_paths: data.part_s3_paths });
    ...
    if (mergedPaths.length === 0 && data.parsed === 0) {
      logger.info("no_output_no_data", { job_id: event.job_id });
      await transition(event.job_id, JobStatus.DONE, undefined, { output_paths: [] });
      return;
    }

    logger.info("transitioning_to_loading", { job_id: event.job_id, merged_paths_count: mergedPaths.length });
    ...
    logger.info("loading_message_sent", { job_id: event.job_id });
  } catch (error) {
    logger.error("finalization_exception", { job_id: event.job_id, error: String(error), stack: error instanceof Error ? error.stack : undefined });
