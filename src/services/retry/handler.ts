...
class RetryService extends ServiceManager {
  protected static instance: RetryService;
  private logger: any;
  ...
  private constructor(enforce: () => void) {
    if (enforce !== Enforce) {
      throw new InstantiationError("Cannot instantiate RetryService directly. Use getInstance()");
    }
    super(enforce);
  ...
  public static getInstance(): RetryService {
    if (!ServiceManager.instance) {
      ServiceManager.instance = new RetryService(Enforce);
    }
    return ServiceManager.instance as RetryService;
  }
