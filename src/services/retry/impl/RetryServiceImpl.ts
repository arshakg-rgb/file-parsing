...
import { DLQMessage, _FailureClass, LoadMessage } from "../../../shared/models/job.js";
...
import { RetryRequest, RetryResponse } from "../io/IRetry.js";
    ...
    if (msg.attempts >= config.settings.RETRY_MAX_ATTEMPTS) 
{
    ...
    if (msg.failure_class === _FailureClass.ENCODING_ERROR) 
{
      recovered = await this.retryEncoding(rawBytes, msg);
    }
 else if ([_FailureClass.TRANSFORM_ERROR, _FailureClass.EXTRACTION_ERROR].includes(msg.failure_class)) 
{
      recovered = await this.retryAfterTemplateUpdate(rawBytes, msg);
    }
 else if (msg.failure_class === _FailureClass.TYPE_MISMATCH) 
{
      recovered = await this.retryBroadCoercion(rawBytes, msg);
    }
 else if (msg.failure_class === _FailureClass.UNCERTAIN) 
{
      await this.markForReview(msg);
      return;
    }
