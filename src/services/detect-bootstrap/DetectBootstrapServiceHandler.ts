...
export class DetectBootstrapService
{
  ...
  private async extractHeaders(bucket: string, key: string, encoding: string, fileSize: number, jobId: string, fieldSpec: string[], extraOffsets: number[] = [], extraWindowSize: number = 0): Promise<{ headers: string[]; fieldMap?: ColumnMap } | null>
  {
    try
    {
      ...
      return { headers: this.expandJsonColumns(rawHeaders, delimiter, dataLines) };
    }
    catch (err)
    {
  ...
  private collectJsonLeafPaths(obj: Record<string, unknown>, prefix: string, into: Set<string>): void
  {
    for (const [k, v] of Object.entries(obj))
    {
      ...
      {
        this.collectJsonLeafPaths(v as Record<string, unknown>, path, into);
      }
      else
      {
        into.add(path);
  ...
  private expandJsonColumns(headers: string[], delimiter: string, dataLines: string[]): string[]
  {
    const quoteChar: string = DetectBootstrapService.csvQuoteFor(delimiter);
    const sampleRows: string[][] = dataLines
      .slice(0, 25)
      .map((line) => DetectBootstrapService.parseCsvLine(line, delimiter, quoteChar))
    ...
    for (let col = 0; col < headers.length; col++)
    {
      ...
      for (const v of values)
      {
        ...
        if (Array.isArray(parsed))
        {
          ...
          this.collectJsonLeafPaths(parsed[0] as Record<string, unknown>, header, leafPaths);
          continue;
        }
        ...
        this.collectJsonLeafPaths(parsed as Record<string, unknown>, header, leafPaths);
      }
