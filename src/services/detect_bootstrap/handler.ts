...
class DetectBootstrapService extends ServiceManager {
  protected static instance: DetectBootstrapService;
  private logger: any;
  ...
  private constructor(enforce: () => void) {
    if (enforce !== Enforce) {
      throw new InstantiationError("Cannot instantiate DetectBootstrapService directly. Use getInstance()");
    }
    super(enforce);
  ...
  public static getInstance(): DetectBootstrapService {
    if (!ServiceManager.instance) {
      ServiceManager.instance = new DetectBootstrapService(Enforce);
    }
    return ServiceManager.instance as DetectBootstrapService;
  }
...
const detectService = DetectBootstrapService.getInstance();
