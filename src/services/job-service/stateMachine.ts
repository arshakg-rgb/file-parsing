...
async function createChildJob(event: JobEvent): Promise<void> {
  ...
  logger.info({ parent: data.parent_job_id, child: childId }, "child_job_created");
}
...
async function onParsingCompleted(event: JobEvent): Promise<void> {
  ...
  logger.info({ job_id: event.job_id, parsed: data.parsed, dropped: data.dropped_rubbish, failed: data.failed, part_count: data.part_s3_paths.length }, "parsing_completed_received");
  
  const row = await getJob(event.job_id);
  if (!row) {
    logger.error({ job_id: event.job_id }, "job_not_found");
    return;
  }
  ...
  logger.info({ job_id: event.job_id, failed_ratio: failedRatio }, "transitioning_to_finalizing");
  ...
  logger.info({ job_id: event.job_id, part_paths: data.part_s3_paths }, "starting_finalization");
  
  try {
    const finalizeResult = await finalizeOutput(event.job_id, data.part_s3_paths, settings.DATA_BUCKET);
    logger.info({ job_id: event.job_id, failed: finalizeResult.failed, paths_count: finalizeResult.paths.length, error: finalizeResult.error }, "finalization_result");
    
    if (finalizeResult.failed) {
      logger.error({ job_id: event.job_id, error: finalizeResult.error }, "finalize_failed");
    ...
    logger.info({ job_id: event.job_id, merged_paths_count: mergedPaths.length, merged_paths: mergedPaths }, "finalize_complete");
    ...
    if (failedRatio > settings.FAILED_LINE_RATIO_THRESHOLD) {
      logger.warn({ job_id: event.job_id, failed_ratio: failedRatio, threshold: settings.FAILED_LINE_RATIO_THRESHOLD }, "quality_gate_held");
    ...
    if (mergedPaths.length === 0 && data.parsed > 0) {
      logger.warn({ job_id: event.job_id, parsed: data.parsed, part_paths: data.part_s3_paths }, "no_output_paths_with_parsed_data");
    ...
    logger.info({ job_id: event.job_id, merged_paths_count: mergedPaths.length }, "transitioning_to_loading");
    ...
    logger.info({ job_id: event.job_id }, "loading_message_sent");
  } catch (error) {
    logger.error({ job_id: event.job_id, error: String(error), stack: error instanceof Error ? error.stack : undefined }, "finalization_exception");
