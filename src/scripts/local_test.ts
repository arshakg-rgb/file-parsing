...
function check(label: string, fn: () => void) 
{
  try 
{
 fn(); _passed++; pass(label); 
}
  catch (e) 
{
 _failed++; fail(label, e); 
}
}
...
{
  if (!url || !url.startsWith("gs://")) throw new Error(`Expected gs:
...
function uploadPath(jobId: string) 
{
 return `gs:
}
function ingestedPath(jobId: string) 
{
 return `gs://${BUCKET}/ingested/${jobId}/source`; 
}
