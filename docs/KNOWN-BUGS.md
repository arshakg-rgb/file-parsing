...
- **`SYSTEM_PROMPT` is never sent to Vertex AI.** In `src/services/ai_classifier/AiClassifierServiceImpl.ts`, `SYSTEM_PROMPT` is defined (line 62) but `callVertexAI` (line 66) calls `askVertexAI(prompt)` with only the user prompt — the system prompt is never included. The `extractJson` leniency (line 119) papers over the resulting sloppier output.
